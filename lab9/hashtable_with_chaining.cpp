...
struct HashTable{
    ...
    void hash_lookup(string user_name){
        ...
        empty = is_slot_empty(hash);
	//add your code below
        if (!empty){  //inform the user that the slot is empty by giving a messege
            cout << "Slot is empty!"<<endl;
        }
        else{                                         
            Node *current_node = password[hash].head;
            while (current_node != NULL){        //iterating over each node of the linked list of the given slot
                if (current_node->user_name==user_name){    //if  found a node with the required username , return the password in that node
                    cout << "User password : "<<current_node->password <<endl;
                    return;
                }
                else{
                    current_node = current_node->link;   //traverse to the enxt node
                }
                cout << "User Name not found!"<<endl; //warn the user if the username is not found in any node of the linked list
            }
        }
    }
};
